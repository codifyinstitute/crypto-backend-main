const usaStaticSchema = require('../Models/usaStaticSchema');
...
exports.addStaticData = async (req, res) => {
    try {
        ...
        const newStaticData = new usaStaticSchema({ TransactionFee, LoginId, Password, MinAmount, NetworkFee, Binance: binance, Coinbase: coinbase, Kraken: kraken, Wazirx: wazirx });
...
exports.getAllStaticData = async (req, res) => {
    try {
        const staticData = await usaStaticSchema.find();
...
exports.getStaticDataById = async (req, res) => {
    try {
        const { id } = req.params;
        const staticData = await usaStaticSchema.findById(id);
...
exports.updateStaticData = async (req, res) => {
    try {
        ...
        const updatedStaticData = await usaStaticSchema.findByIdAndUpdate(
            id,
...
exports.deleteStaticData = async (req, res) => {
    try {
        ...
        const deletedStaticData = await usaStaticSchema.findByIdAndDelete(id);
...
exports.loginUser = async (req, res) => {
    ...
    try {
        const user = await usaStaticSchema.findOne({ LoginId });
        if (!user) {
