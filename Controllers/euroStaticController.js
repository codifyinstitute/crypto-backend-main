const euroStaticSchema = require('../Models/euroStaticSchema');
...
exports.addStaticData = async (req, res) => {
    try {
        ...
        const newStaticData = new euroStaticSchema({ TransactionFee, LoginId, Password, MinAmount, NetworkFee, Binance: binance, Coinbase: coinbase, Kraken: kraken, Wazirx: wazirx });

        await newStaticData.save();

        res.status(201).json({ message: "euro data added successfully", data: newStaticData });
    } catch (error) {
        res.status(500).json({ message: "euro adding static data", error: error.message });
    }
};
...
exports.getAllStaticData = async (req, res) => {
    try {
        const staticData = await euroStaticSchema.find();
...
exports.getOneStaticData = async (req, res) => {
    try {
        const staticData = await euroStaticSchema.findOne();
...
exports.getStaticDataById = async (req, res) => {
    try {
        const { id } = req.params;
        const staticData = await euroStaticSchema.findById(id);
...
exports.updateStaticData = async (req, res) => {
    try {
        ...
        const updatedStaticData = await euroStaticSchema.findByIdAndUpdate(
            id,
...
exports.deleteStaticData = async (req, res) => {
    try {
        ...
        const deletedStaticData = await euroStaticSchema.findByIdAndDelete(id);
...
exports.loginUser = async (req, res) => {
    ...
    try {
        const user = await euroStaticSchema.findOne({ LoginId });
        if (!user) {
